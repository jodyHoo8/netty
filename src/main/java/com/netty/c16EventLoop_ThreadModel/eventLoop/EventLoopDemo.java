...
/**
    Netty事件循环EventLoop
    事件循环的意思就是：它运行在一个循环中，直到它停止。
    网络框架需要需要在一个循环中为一个特定的连接运行事件，
    所以这符合网络框架的设计。在Netty之前，已经有其他框架和实现这么做了。

    下面的清单显示了典型的 EventLoop 逻辑。
    请注意这是为了更好的说明这个想法而不是单单展示 Netty 实现本身。

    EventLoop 是完全由一个 Thread,从未改变。为了更合理利用资源,根据配置和可用的内核，
    Netty 可以使用多个 EventLoop。
    事件/任务执行顺序
    一个重要的细节关于事件和任务的执行顺序是,事件/任务执行顺序按照FIFO(先进先出)。
    这是必要的,因为否则事件不能按顺序处理,所处理的字节将不能保证正确的顺序。这将导致问题,所以这个不是所允许的设计。
...
public class EventLoopDemo {
    public static void main(String[] args) {
        while (!false/*terminated*/) {
            List<Runnable> readyEvents = null;
            //blockUntilEventsReady(); //1
